...
export default function ProductCard({ product, featuredBadge, priority = false }: ProductCardProps) {
  ...
  return (
    <div
      ...
      className="group relative flex flex-col rounded-3xl overflow-hidden will-change-transform"
      style={{
    ...
    >
      ...
      <div 
        className={`absolute inset-0 rounded-3xl pointer-events-none transition-all duration-500 ${
          isHovered
            ...
            : 'border border-stone-200/90 dark:border-stone-800/90 shadow-md shadow-stone-900/5 dark:shadow-stone-950/40'
        }`}
      />
      ...
      {/* ========================================================================= */}
      <div className="relative flex flex-col flex-1 bg-white dark:bg-[#181715] rounded-3xl overflow-hidden">
        ...
        <Link
          ...
          className="relative aspect-square w-full bg-sand-100/80 dark:bg-[#201F1B] overflow-hidden block"
        >
          {product.image_url ? (
            <Image
          ...
          <div className="absolute top-3 right-3 left-3 z-20 flex items-center justify-between pointer-events-none">
            <span className="px-2.5 py-1 text-[11px] font-bold bg-white/95 dark:bg-stone-900/95 backdrop-blur-md text-stone-800 dark:text-sand-100 rounded-full border border-stone-200/60 dark:border-stone-700/60 shadow-xs pointer-events-auto">
              {badge}
            </span>
          ...
          {isOutOfStock && (
            <div className="absolute inset-0 bg-stone-950/65 backdrop-blur-[2px] flex items-center justify-center z-30">
              <span className="px-3.5 py-1.5 rounded-full bg-rose-600 text-white text-xs font-bold shadow-lg tracking-wide">
                نفذت الكمية حالياً
              </span>
            </div>
          )}
        ...
        {/* ========================================================================= */}
        <div className="p-4 sm:p-5 flex flex-col flex-1 justify-between gap-3 relative z-10 bg-white dark:bg-[#181715]">
          <div>
            {/* Category & Craft Rating */}
            <div className="flex items-center justify-between text-[11px] text-stone-500 dark:text-stone-400 mb-1.5 font-semibold">
            ...
            >
              <h3 className="font-extrabold text-stone-900 dark:text-sand-100 text-sm sm:text-base leading-snug line-clamp-1">
                {product.name_ar}
              </h3>
            </Link>

            {/* Short Genuine Description */}
            <p className="text-[11px] text-stone-500 dark:text-stone-400 mt-1 line-clamp-1 leading-relaxed">
          ...
          <div className="pt-3 border-t border-stone-100 dark:border-stone-800/80 flex items-center justify-between gap-2">
            <div className="flex flex-col">
              <span className="text-[9px] uppercase font-bold text-stone-400 dark:text-stone-500 leading-none mb-0.5">السعر</span>
              <span className="text-base sm:text-lg font-black text-stone-950 dark:text-white font-mono tracking-tight">
            ...
            {isOutOfStock ? (
              ...
              <button
                ...
                className={`h-10 sm:h-11 px-3 sm:px-4 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all duration-200 active:scale-95 shadow-xs ${
                  isInCart
              ...
              >
                {isInCart ? (
                  ...
                  <>
                    ...
                    <span className="whitespace-nowrap">اختاري القطعة</span>
                  </>
                )}
              </button>
            )}
